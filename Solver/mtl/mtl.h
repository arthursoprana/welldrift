...
struct abs_add {
  ...
  T operator()(const T& a, const U& b) {
    return a + fabs(b);//MTL_ABS(b); jaime-sinmec-15.06.07
  }
};
...
struct sqr_add { 
  template <class T, class U>
  T operator()(const T& a, const U& b) {
    return a + fabs(b * b);//MTL_ABS(b * b);//sinmec-jaime14.06.07
  }
};
...
struct sqr_ { 
  template <class T, class U>
  T operator()(const T& a, const U& b) {
    return a + fabs(b * b);//MTL_ABS(b * b); jiame sinmec - 15.06.07
  }
};
