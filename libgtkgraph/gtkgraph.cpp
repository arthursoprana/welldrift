...
static void gtk_graph_class_init (GtkGraphClass *aClass);
...
static void gtk_graph_class_init (GtkGraphClass *aClass)
{
    ...
    parent_class = reinterpret_cast<GtkWidgetClass*>(gtk_type_class (gtk_widget_get_type ()));
...
GtkWidget* gtk_graph_new (GtkGraphType type)
{
  ...
  widget = reinterpret_cast<GtkWidget*>(gtk_type_new (gtk_graph_get_type ()));
...
if (gtk_widget_get_realized (widget)) // Then we are moving / resizing an existing widget
...
static void gtk_graph_destroy (GtkObject *object)
{
	...
	free (graph->dependant);
	free (graph->independant);
    g_free (graph->traces);
    /* --- Call parent destroy --- */
...
g_return_if_fail (GTK_IS_GRAPH (graph));
  ...
  if (gtk_widget_get_realized (GTK_WIDGET(graph)))
    {
...
for (i = 0 ; i <= graph->dependant->n_maj_tick ; i++)
    ...
    float axis_num = graph->dependant->axis_max - i*graph->dependant->maj_tick;
    axis_num = abs(axis_num) > 1.0e-7 ? axis_num : 0.0;
	tbuffer = g_strdup_printf(format_string, axis_num );
	g_free(format_string);
    if (graph->dependant->grid_visible){
        ...
        gdk_draw_line(buffer, Gridcontext, user_origin_x, y_coord, user_origin_x + graph->independant->pxls_per_maj_tick * graph->independant->n_maj_tick, y_coord);
    }
